...
if __name__ == '__main__':
    ...
    for frame_number in range(100):
        ...
        end_time = time.time()
        inference_delays.append(end_time - start_time)
        print(f"Inference time: {1_000 * (end_time- start_time):.0f} ms")
        print(f"Predictions: {predictions}")
    video_reader.close()
    print(f"Average inference time: {1_000 * np.mean(inference_delays):.1f} ms, median: {1_000 * np.median(inference_delays):.1f} ms")